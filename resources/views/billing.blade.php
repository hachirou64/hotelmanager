...
<head>
    ...
    <title>Facturation - Hôtel Manager</title>
...
<body class="bg-gray-50 dark:bg-slate-900 {{ auth()->user()->preferences['theme'] ?? 'auto' === 'dark' ? 'dark-theme' : '' }}">
    ...
    <div class="flex pt-16">
        ...
        <aside id="sidebar" class="fixed md:static top-16 left-0 w-64 bg-white dark:bg-slate-800 shadow-sm min-h-screen transform -translate-x-full md:translate-x-0 transition-transform duration-300 ease-in-out z-30">
            <nav class="mt-8">
                <div class="px-4 space-y-2">
                    ...
                    <!-- Facturation -->
                    <a href="{{ route('billing') }}" class="flex items-center px-4 py-2 text-sm font-medium text-white bg-primary rounded-md">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <main class="flex-1 p-4 md:p-8">
            <!-- Header Section -->
            <div class="mb-8">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Facturation</h1>
                <p class="text-gray-600 dark:text-gray-300">Gérez les factures de votre hôtel.</p>
            </div>

            <!-- Invoices List -->
            <div class="bg-white dark:bg-slate-800 shadow rounded-lg">
                <div class="px-4 py-5 sm:p-6">
                    <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white mb-4">Liste des Factures</h3>
                    <div class="space-y-3">
                        @forelse($invoices as $invoice)
                        <div class="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-600 rounded-lg">
                            <div>
                                <p class="text-sm font-medium text-gray-900 dark:text-white">Facture #{{ $invoice->id_facture }}</p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Client: {{ $invoice->client->nom }} {{ $invoice->client->prenom }}</p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Date: {{ $invoice->date_facture }}</p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Montant: {{ $invoice->montant_total }} €</p>
                            </div>
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                @if($invoice->statut_paiement === 'payée') bg-green-100 text-green-800
                                @else bg-red-100 text-red-800
                                @endif">
                                {{ ucfirst($invoice->statut_paiement) }}
                            </span>
                        </div>
                        @empty
                        <p class="text-sm text-gray-500 dark:text-gray-400">Aucune facture trouvée</p>
                        @endforelse
                    </div>
                </div>
            </div>

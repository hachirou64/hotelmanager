...
<head>
    ...
    <title>Personnel - Hôtel Manager</title>
...
<body class="bg-gray-50 dark:bg-slate-900 {{ auth()->user()->preferences['theme'] ?? 'auto' === 'dark' ? 'dark-theme' : '' }}">
    ...
    <div class="flex pt-16">
        ...
        <aside id="sidebar" class="fixed md:static top-16 left-0 w-64 bg-white dark:bg-slate-800 shadow-sm min-h-screen transform -translate-x-full md:translate-x-0 transition-transform duration-300 ease-in-out z-30">
            <nav class="mt-8">
                <div class="px-4 space-y-2">
                    ...
                    <!-- Personnel -->
                    <a href="{{ route('personnel') }}" class="flex items-center px-4 py-2 text-sm font-medium text-white bg-primary rounded-md">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <main class="flex-1 p-4 md:p-8">
            <!-- Header Section -->
            <div class="mb-8">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Gestion du Personnel</h1>
                <p class="text-gray-600 dark:text-gray-300">Gérez le personnel de votre hôtel.</p>
            </div>

            <!-- Personnel List -->
            <div class="bg-white dark:bg-slate-800 shadow rounded-lg">
                <div class="px-4 py-5 sm:p-6">
                    <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white mb-4">Liste du Personnel</h3>
                    <div class="space-y-3">
                        @forelse($personnels as $personnel)
                        <div class="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-600 rounded-lg">
                            <div>
                                <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $personnel->nom }} {{ $personnel->prenom }}</p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Rôle: {{ $personnel->role }}</p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Service: {{ $personnel->service }}</p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Utilisateur: {{ $personnel->user->email ?? 'N/A' }}</p>
                            </div>
                        </div>
                        @empty
                        <p class="text-sm text-gray-500 dark:text-gray-400">Aucun personnel trouvé</p>
                        @endforelse
                    </div>
                </div>
            </div>
